...
if ($id) {
    try {
        $stmt = $pdo->prepare("SELECT * FROM documentos_glosa WHERE id = ?");
        $stmt->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($data_cadastro) || empty($convenio_id)) {
        ...
        try {
            if ($id) {
                // Atualizar documento
                $sql = "UPDATE documentos_glosa SET data_cadastro=?, competencia=?, convenio_id=?, observacoes=? WHERE id=?";
                $stmt = $pdo->prepare($sql);
                $stmt->execute([$data_cadastro, $competencia, $convenio_id, $observacoes, $id]);
                $documento_id = $id;
            } else {
                // Inserir novo documento
                $sql = "INSERT INTO documentos_glosa (data_cadastro, competencia, convenio_id, observacoes) VALUES (?, ?, ?, ?)";
                $stmt = $pdo->prepare($sql);
            ...
            if (!empty($_FILES['anexos']['name'][0])) {
                $upload_dir = 'uploads/documentos_glosa/';
                ...
                for ($i = 0; $i < $total_files; $i++) {
                    if ($_FILES['anexos']['error'][$i] == 0) {
                        ...
                        if (move_uploaded_file($_FILES['anexos']['tmp_name'][$i], $caminho_destino)) {
                            ...
                            $sql_anexo = "INSERT INTO documentos_glosa_anexos (documento_id, nome_arquivo, caminho_arquivo, tamanho_arquivo, tipo_arquivo) VALUES (?, ?, ?, ?, ?)";
            ...
            if (!$id) {
                echo "<script>setTimeout(function(){ window.location.href = 'documentos.php'; }, 1500);</script>";
            } else {
                // Recarregar anexos
                $stmt_anexos = $pdo->prepare("SELECT * FROM documentos_glosa_anexos WHERE documento_id = ? ORDER BY created_at DESC");
                $stmt_anexos->execute([$id]);
...
<div class="container mx-auto px-4 py-6 max-w-4xl">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800"><?php echo $id ? 'Editar' : 'Novo'; ?> Documento</h1>
                        ...
                        <?php foreach ($anexos_existentes as $anexo): ?>
                            <div class="flex items-center justify-between bg-gray-50 p-3 rounded-lg border border-gray-200">
                                ...
                                <div class="flex gap-2">
                                    ...
                                    <a href="excluir_anexo_documento.php?id=<?php echo $anexo['id']; ?>&doc_id=<?php echo $id; ?>" onclick="return confirm('Deseja excluir este anexo?');" class="text-red-600 hover:text-red-800" title="Excluir">
                                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
