...
if ($filtro_guia) {
    $where_clauses[] = "i.guia_paciente LIKE ?";
    $params[] = "%$filtro_guia%";
}
...
<div class="w-full px-2 mt-4">
    <div class="flex justify-between items-center mb-6 px-2">
        <h2 class="text-2xl font-bold text-gray-800">Gestão de Atendimento</h2>
        <div class="flex gap-2">
            <a href="consolidar_atendimentos.php" class="bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline transition duration-150 ease-in-out">
                ⚡ Consolidar para Relatório Mensal
            </a>
            <a href="registrar_internacao.php" class="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline transition duration-150 ease-in-out">
                + Novo Atendimento
            </a>
        </div>
    </div>
    ...
    <div class="bg-white p-4 rounded-lg shadow border border-gray-200 mb-6">
        <form method="GET" class="flex flex-wrap items-end gap-4">
            <div>
                <label for="filtro_paciente" class="block text-sm font-medium text-gray-700 mb-1">Paciente</label>
                <input type="text" name="filtro_paciente" id="filtro_paciente" value="<?php echo htmlspecialchars($filtro_paciente); ?>" class="border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Nome do paciente">
            </div>
            <div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-1 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Paciente</th>
                    ...
                    <th class="px-1 py-2 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Conta</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <?php foreach ($internacoes as $int): ?>
                        <tr class="hover:bg-gray-50">
                            <td class="px-1 py-2 whitespace-nowrap text-xs text-gray-500">
                                <?php echo $int['competencia'] ? date('m/Y', strtotime($int['competencia'])) : '-'; ?>
                            </td>
                            <td class="px-1 py-2 whitespace-nowrap text-xs text-gray-500">
                                <?php echo $int['data_recebimento'] ? date('d/m/Y', strtotime($int['data_recebimento'])) : '-'; ?>
                            </td>
                            <td class="px-1 py-2 text-xs font-medium text-gray-900 max-w-[150px] break-words"><?php echo htmlspecialchars($int['paciente']); ?></td>
                            <td class="px-1 py-2 text-xs text-gray-500 max-w-[100px] break-words"><?php echo htmlspecialchars($int['nome_convenio']); ?></td>
                            <td class="px-1 py-2 whitespace-nowrap text-xs text-gray-500"><?php echo htmlspecialchars($int['guia_paciente']); ?></td>
                            <td class="px-1 py-2 whitespace-nowrap text-xs text-gray-500">
                                <div>E: <?php echo $int['data_entrada'] ? date('d/m/Y', strtotime($int['data_entrada'])) : '-'; ?></div>
                                <div>S: <?php echo $int['data_saida'] ? date('d/m/Y', strtotime($int['data_saida'])) : '-'; ?></div>
                            </td>
                            <td class="px-1 py-2 whitespace-nowrap text-xs text-gray-900 text-right">
                                <?php echo number_format($int['valor_inicial'], 2, ',', '.'); ?>
                            </td>
                            <td class="px-1 py-2 whitespace-nowrap text-xs text-red-600 text-right">
                                <?php echo number_format($int['valor_retirado'], 2, ',', '.'); ?>
                            </td>
                            <td class="px-1 py-2 whitespace-nowrap text-xs text-green-600 text-right">
                                <?php echo number_format($int['valor_acrescentado'], 2, ',', '.'); ?>
                            </td>
                            <td class="px-1 py-2 whitespace-nowrap text-xs text-gray-900 text-right font-bold">
                                <?php echo number_format($int['valor_total'], 2, ',', '.'); ?>
                            </td>
                            <td class="px-1 py-2 whitespace-nowrap text-xs text-gray-500 text-right">
                                <?php echo number_format($int['valor_glosado'], 2, ',', '.'); ?>
                            </td>
                            <td class="px-1 py-2 whitespace-nowrap text-xs text-gray-500 text-right">
                                <?php echo number_format($int['valor_aceito'], 2, ',', '.'); ?>
                            </td>
                            <td class="px-1 py-2 whitespace-nowrap text-xs text-gray-500 text-right">
                                <?php echo number_format($int['valor_faturado'], 2, ',', '.'); ?>
                            </td>
                            <td class="px-1 py-2 whitespace-nowrap text-center text-xs">
                                <?php if ($int['conta_corrigida']): ?>
                                    <a href="<?php echo $int['conta_corrigida']; ?>" target="_blank" class="text-blue-600 hover:underline">Ver</a>
                                <?php else: ?>
                                    -
                                <?php endif; ?>
                            </td>
                            <td class="px-1 py-2 whitespace-nowrap text-center text-xs text-gray-500">
                                <?php echo htmlspecialchars($int['falta_nf']); ?>
                            </td>
                            <td class="px-1 py-2 whitespace-nowrap text-center">
                                <span class="px-1 inline-flex text-[10px] leading-4 font-semibold rounded-full 
                                    <?php 
                                        if($int['status'] == 'Em Aberto') echo 'bg-yellow-100 text-yellow-800';
                                        elseif($int['status'] == 'Auditado') echo 'bg-blue-100 text-blue-800';
                                        elseif($int['status'] == 'Fechado') echo 'bg-green-100 text-green-800';
                                        else echo 'bg-gray-100 text-gray-800';
                                    ?>">
                                    <?php echo htmlspecialchars($int['status']); ?>
                                </span>
                            </td>
                            <td class="px-1 py-2 whitespace-nowrap text-center text-xs font-medium">
                                <div class="flex justify-center items-center">
                                    <button onclick='verObservacao(<?php echo json_encode($int['observacoes']); ?>)' class="<?php echo !empty($int['observacoes']) ? 'text-orange-500 hover:text-orange-700' : 'text-gray-400 hover:text-gray-600'; ?> mr-1" title="Ver Observações">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                        </svg>
                                    </button>
                                    <a href="registrar_internacao.php?id=<?php echo $int['id']; ?>" class="text-indigo-600 hover:text-indigo-900 mr-3" title="Editar">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                        </svg>
                                    </a>
                                    <a href="javascript:void(0);" onclick="if(confirm('Tem certeza que deseja excluir este atendimento?')) window.location.href='excluir_internacao.php?id=<?php echo $int['id']; ?>';" class="text-red-600 hover:text-red-900" title="Excluir">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="7" class="px-6 py-2 text-center text-gray-500">Nenhum atendimento encontrado.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
...
<script>
    ...
    function fecharModal(event) {
        // Se o evento for passado (clique no background), verifica se é o background
